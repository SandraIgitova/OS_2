...
int main(){
	...
	if(error1 == -1 || error2 == -1){
		printf("Can not create pipe(s). Exiting with status \n");
		exit(-1);
	}
	...
	if(sp < 0){
		printf("Не может создать новый процесс. \n");
		exit(-1);
	}else if(sp > 0){
		//parent
		...
		while(true){
			...
			int len = read(STDIN_FILENO, arr, 100);
			arr[len - 1] = '\0';

			write(toChild[WRITE], arr, strlen(arr) + 1); 


			len = read(toParent[READ], arr, 100); 
			if(len > 1) {
				printf("%s\n", arr);
			} else {
				break;
		...
		queue *q = create();
		while(true){
			int len = read(toChild[READ], arr, 100);
			int x = checkCommand(arr);
			if(x == -1){
				write(toParent[WRITE], "Ошибка: некорректная команда.\n", 30); 
			}else{
				switch(x){
					case 0:
						write(toParent[WRITE], "q", 1); 
						break;
					case 1:
					...
					case 3:
						...
						break;
					case 4:
						push(q, slice(arr, 5, strlen(arr)-1)); 
						write(toParent[WRITE], "\0\0", 2);
						break;
					case 5:
						pop(q);
